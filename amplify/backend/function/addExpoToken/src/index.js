...
exports.handler = async (event) => {
	...
	// 1. Check if the token already exists and belongs to logged in user
	...
	const tokenResponse = await graphqlClient.query({
		query: getExpoToken,
		variables: {
	...
	const tokenObj = tokenResponse.data.getExpoToken;
	if (tokenObj) {
		if (tokenObj.playerUsername === player) {
			return tokenObj;
		} else {
			// 2. If the token exists but belongs to another user, update the token
			...
			const updateTokenResponse = await graphqlClient.mutate({
			...
			return updateTokenResponse.data.updateExpoToken;
		}
	} else {
		...
		const createTokenResponse = await graphqlClient.mutate({
		...
		return createTokenResponse.data.createExpoToken;
	}
};
